//! All iced `Subscription`-returning free functions: one per Convex live
//! query (friends, servers, channels, members, conversations, calls, ...),
//! plus the tray-icon bridge and a couple of one-shot background `Task`s
//! (`mark_read_task`, `typing_ping_task`) that fit better here than in
//! update.rs. Each Convex subscription follows the same shape: open a
//! `client.subscribe(...)`, loop over pushes, parse into a domain type,
//! forward as a `Message` into the iced runtime.
...
pub(crate) fn roles_subscription(
    ...
    Subscription::run_with_id(
        ("roles-subscription", server_id.clone()),
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let roles = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::ServerRolesUpdated(roles)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn my_perms_subscription(
    ...
    Subscription::run_with_id(
        ("my-perms-subscription", server_id.clone()),
        iced::stream::channel(8, move |mut output| {
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    ...
                    if output
                        .send(Message::MyServerPermsUpdated(perms))
                        .await
                        .is_err()
                    {
                        break;
...
pub(crate) fn room_voice_subscription(
    ...
    Subscription::run_with_id(
        ...
        iced::stream::channel(16, move |mut output| {
            ...
            let noise_gate = Arc::clone(&noise_gate);
            async move {
                ...
                while let Some(event) = event_rx.next().await {
                    if output
                        .send(Message::RoomVoiceEngineEvent(event))
                        .await
                        .is_err()
                    {
                        break;
...
pub(crate) fn voice_users_subscription(
    ...
    Subscription::run_with_id(
        ("voice-users-subscription", conversation_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let conversation_id = conversation_id.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let users = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::VoiceUsersUpdated(users)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn friends_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let friends = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::FriendsUpdated(friends)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn servers_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let servers = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::ServersUpdated(servers)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn channels_subscription(
    ...
    Subscription::run_with_id(
        ("channels-subscription", server_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let server_id = server_id.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let channels = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::ChannelsUpdated(channels)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn members_subscription(
    ...
    Subscription::run_with_id(
        ("members-subscription", server_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let server_id = server_id.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let members = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::MembersUpdated(members)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn tray_subscription() -> Subscription<Message> {
    Subscription::run_with_id(
        ...
        iced::stream::channel(16, move |mut output| async move {
            let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<tray::TrayEvent>();
            tray::spawn(tx);
...
pub(crate) fn requests_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let requests = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::RequestsUpdated(requests)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn outgoing_requests_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let requests = parse_object_array(result)
                        ...
                        .collect();
                    if output
                        .send(Message::OutgoingRequestsUpdated(requests))
                        .await
                        .is_err()
                    {
                        break;
...
pub(crate) fn social_stats_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
        "social-stats-subscription",
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    if let FunctionResult::Value(Value::Object(obj)) = result {
                        ...
                        if output.send(Message::SocialStatsUpdated(stats)).await.is_err() {
                            break;
                        }
                    }
                }
...
pub(crate) fn suggestions_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let list = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::SuggestionsUpdated(list)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn blocked_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let blocked = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::BlockedUpdated(blocked)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn conversations_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
...
pub(crate) fn admin_users_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
            ...
            async move {
                ...
                while let Some(result) = sub.next().await {
                    let users = parse_object_array(result)
                        ...
                        .collect();
                    if output.send(Message::AdminUsersUpdated(users)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn my_call_subscription(client: ConvexClient, token: String) -> Subscription<Message> {
    Subscription::run_with_id(
        "my-call-subscription",
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    ...
                    if output.send(Message::MyCallUpdated(info)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn call_subscription(
    ...
    Subscription::run_with_id(
        ...
        iced::stream::channel(16, move |mut output| {
            ...
            let output_muted = Arc::clone(&output_muted);
            let noise_gate = Arc::clone(&noise_gate);
            let share_control_slot = Arc::clone(&share_control_slot);
            async move {
                ...
                // Runs once per call (`Subscription::run_with_id` dedups by
                // key), so this is the one place that ever takes the
                // receiver out of the slot.
                ...
                while let Some(event) = event_rx.next().await {
                    if output.send(Message::CallEngineEvent(event)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn messages_subscription(
    ...
    Subscription::run_with_id(
        ("messages-subscription", conversation_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let conversation_id = conversation_id.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    ...
                    // runs on the UI thread where the ratchet session lives.
                    let messages = parse_object_array(result)
                        ...
                        .map(|obj| {
                            ...
                            // decrypt cache / ratchet on the UI thread.
                            let reply_to = obj_object(&obj, "replyTo").map(|r| {
                                (obj_str(&r, "authorName"), obj_str(&r, "snippet"))
                            });
                        ...
                        .collect();
                    if output.send(Message::MessagesUpdated(messages)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn typing_subscription(
    ...
    Subscription::run_with_id(
        ("typing-subscription", conversation_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let conversation_id = conversation_id.clone();
            async move {
                ...
                while let Some(result) = sub.next().await {
                    ...
                    if output.send(Message::TypingUpdated(names)).await.is_err() {
                        break;
                    }
                }
            }
...
pub(crate) fn peer_session_subscription(
    ...
    Subscription::run_with_id(
        ("peerseal-session", peer_user_id.clone()),
...
pub(crate) fn peer_invite_subscription(
    ...
    Subscription::run_with_id(
        ("peerseal-invite", peer_user_id.clone()),
        iced::stream::channel(16, move |mut output| {
            let mut client = client.clone();
            let token = token.clone();
            let peer_user_id = peer_user_id.clone();
